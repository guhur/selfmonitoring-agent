...
parser = argparse.ArgumentParser(
    description="PyTorch for Matterport3D Agent with panoramic view and action"
)
# General options
parser.add_argument(
    "--exp_name",
    default="experiments_",
    type=str,
...
parser.add_argument(
    ...
    type=str,
    help="path to training vocab",
)
parser.add_argument(
    "--trainval_vocab",
    default="tasks/R2R-pano/data/trainval_vocab.txt",
    type=str,
    help="path to training and validation vocab",
)
parser.add_argument(
    "--img_feat_dir",
    default="img_features/ResNet-152-imagenet.tsv",
    type=str,
    help="path to pre-cached image features",
)
...
parser.add_argument(
    "--max_episode_len", default=10, type=int, help="maximum length of episode"
)
parser.add_argument(
    "--feedback_training",
    default="sample",
...
parser.add_argument(
    "--mlp_relu", default=1, type=int, help="Use ReLu in MLP module"
)
parser.add_argument(
    "--img_fc_use_angle",
    default=1,
...
parser.add_argument(
    "--lang_embed", default="lstm", type=str, help="options: lstm "
)
parser.add_argument(
    "--word_embedding_size",
    default=256,
...
parser.add_argument(
    "--max_cap_length", default=80, type=int, help="maximum length of captions"
)
...
parser.add_argument(
    ...
    type=str,
    help="where to save the output results for computing accuracy",
)
parser.add_argument(
    "--resume",
    default="",
...
parser.add_argument(
    ...
    type=str,
    help="path to tensorboard log files",
)
...
def main(opts):
    ...
    if "lstm" in opts.lang_embed:
        ...
        raise ValueError(
            "Unknown {} language embedding".format(opts.lang_embed)
        )
    print(encoder)
    ...
    if opts.arch == "self-monitoring":
        ...
        raise ValueError(
            "Unknown {} model for seq2seq agent".format(opts.arch)
        )
    print(model)
    ...
    # setup trainer
    trainer = PanoSeq2SeqTrainer(
        opts, agent, optimizer, opts.train_iters_epoch
    )
        ...
        if epoch % opts.eval_every_epochs == 0:
            ...
            if is_experiment():
                ...
                best_success_rate = max(
                    success_rate_compare, best_success_rate
                )
                print(
                    "--> Highest val_unseen success rate: {}".format(
                        best_success_rate
                    )
                )
