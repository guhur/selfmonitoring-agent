...
class PanoBaseAgent(object):
    ...
    def _get_distance(self, ob):
        ...
        distance = self.env.distances[ob["scan"]][ob["viewpoint"]][
            gt["path"][-1]
        ]
        return distance

    def _select_action(
        self, logit, ended, is_prob=False, fix_action_ended=True
    ):
        ...
        if self.feedback == "argmax":
            ...
            raise ValueError(
                "Invalid feedback option: {}".format(self.feedback)
            )
    ...
    def _next_viewpoint(self, obs, viewpoints, navigable_index, action, ended):
        ...
        for i, ob in enumerate(obs):
            if action[i] >= 1:
                next_viewpoint_idx.append(
                    navigable_index[i][action[i] - 1]
                )  # -1 because the first one in action is 'stop'
            else:
            ...
            next_headings.append(
                ob["navigableLocations"][next_viewpoints[i]]["heading"]
            )

        return next_viewpoints, next_headings, next_viewpoint_idx, ended

    def pano_navigable_img(self, obs, ended):

        # Get the 36 image features for the panoramic view (including top, middle, bottom)
        num_feature, num_channels, height, width = obs[0]["rgb"].shape

        pano_img = torch.zeros(len(obs), num_feature, num_channels, height, width)
        navigable_img = torch.zeros(
            len(obs), self.opts.max_navigable, num_channels, height, width
        )

        navigable_feat_index, target_index, viewpoints = [], [], []
        for i, ob in enumerate(obs):
            pano_img[i, :] = torch.from_numpy(ob["rgb"])
            ...
            for j, viewpoint_id in enumerate(ob["navigableLocations"]):
                index_list.append(
                    int(ob["navigableLocations"][viewpoint_id]["index"])
                )
            ...
            navigable_img[i, 1 : len(navi_index) + 1] = pano_img[
                i, navi_index
            ]
        ...
        return (
            pano_img,
            navigable_img,
            (viewpoints, navigable_feat_index, target_index),
        )
    ...
    def pano_navigable_feat(self, obs, ended):
        ...
        navigable_feat = torch.zeros(
            len(obs), self.opts.max_navigable, feature_size
        )

        navigable_feat_index, target_index, viewpoints = [], [], []
        for i, ob in enumerate(obs):
            pano_img_feat[i, :] = torch.from_numpy(
                ob["feature"]
            )  # pano feature: (batchsize, 36 directions, 2048)
            ...
            navigable_feat[i, 1 : len(navi_index) + 1] = pano_img_feat[
                i, navi_index
            ]
...
class PanoSeq2SeqAgent(PanoBaseAgent):
    ...
    def __init__(
        ...
        self.ignore_index = (
            opts.max_navigable + 1
        )  # we define (max_navigable+1) as ignore since 15(navigable) + 1(STOP)
        self.criterion = nn.CrossEntropyLoss(ignore_index=self.ignore_index)

        self.MSELoss = nn.MSELoss()
        self.MSELoss_sum = nn.MSELoss(reduction="sum")

    def get_value_loss_from_start(
        self, traj, predicted_value, ended, norm_value=True, threshold=5
    ):
        """
        ...
        if self.opts.mse_sum:
            return (
                self.MSELoss_sum(predicted_value.squeeze(), value_target)
                / sum(1 - ended).item()
            )
        else:
            return self.MSELoss(predicted_value.squeeze(), value_target)

    def get_value_loss_from_start_sigmoid(
        self, traj, predicted_value, ended, norm_value=True, threshold=5
    ):
        """
        This loss forces the agent to estimate how good is the current state, i.e. how far away I am from the goal?
        ...
        for i, _traj in enumerate(traj):
            ...
            dist_improved_from_start = (
                0 if dist_improved_from_start < 0 else dist_improved_from_start
            )
            ...
            traj.append(
                {
                    ...
                    "distance": [
                        super(PanoSeq2SeqAgent, self)._get_distance(ob)
                    ],
                }
            )
    ...
    def update_traj(
        ...
        for i, ob in enumerate(obs):
            if not ended[i] or not last_recorded[i]:
                ...
                traj[i]["img_attn"].append(
                    img_attn[i].detach().cpu().numpy().tolist()
                )
                traj[i]["ctx_attn"].append(
                    ctx_attn[i].detach().cpu().numpy().tolist()
                )
                ...
                if action_prob is not None:
                    traj[i]["action_confidence"].append(
                        action_prob[i].detach().cpu().item()
                    )
                    traj[i]["progress_monitor"].append(
                        (action_prob[i] * ((value[i] + 1) / 2))
                        .detach()
                        .cpu()
                        .item()
                    )
    ...
    def rollout_monitor(self):
        ...
        if (
            self.opts.arch == "progress_aware_marker"
            or self.opts.arch == "iclr_marker"
        ):
            pre_feat = torch.zeros(
                batch_size, self.opts.img_feat_input_dim + self.opts.tiled_len
            ).to(self.device)
        else:
            pre_feat = torch.zeros(
                batch_size, self.opts.img_feat_input_dim
            ).to(self.device)

        # Mean-Pooling over segments as previously attended ctx
        pre_ctx_attend = torch.zeros(batch_size, self.opts.rnn_hidden_size).to(
            self.device
        )
        ...
        for step in range(self.opts.max_episode_len):
            ...
            _, navigable_feat, viewpoints_indices = super(
                PanoSeq2SeqAgent, self
            ).pano_navigable_feat(obs, ended)

            viewpoints, navigable_index, target_index = viewpoints_indices

            _, navigable_img, _ = super().pano_navigable_img(obs, ended)
            navigable_img = navigable_img.to(self.device)

            navigable_ang_feat = navigable_feat[..., 2048:]
            navigable_ang_feat = navigable_ang_feat.to(self.device)
            ...
            action = super(PanoSeq2SeqAgent, self)._select_action(
                logit, ended, fix_action_ended=self.opts.fix_action_ended
            )
            ...
            if not self.opts.test_submission:
                if step == 0:
                    ...
                    if self.opts.monitor_sigmoid:
                        current_val_loss = self.get_value_loss_from_start_sigmoid(
                            traj, value, ended
                        )
                    else:
                        current_val_loss = self.get_value_loss_from_start(
                            traj, value, ended
                        )

                    self.value_loss += current_val_loss
                    current_loss = (
                        self.opts.value_loss_weight * current_val_loss
                        + (1 - self.opts.value_loss_weight)
                        * current_logit_loss
                    )
            else:
                current_loss = torch.zeros(
                    1
                )  # during testing where we do not have ground-truth, loss is simply 0
            loss += current_loss

            next_viewpoints, next_headings, next_viewpoint_idx, ended = super(
                PanoSeq2SeqAgent, self
            )._next_viewpoint(obs, viewpoints, navigable_index, action, ended)
            ...
            traj, last_recorded = self.update_traj(
                obs,
                traj,
                img_attn,
                ctx_attn,
            ...
            pre_feat = navigable_feat[
                torch.LongTensor(range(batch_size)), action, :
            ]
    ...
    def rollout(self):
        ...
        pre_feat = torch.zeros(batch_size, obs[0]["feature"].shape[1]).to(
            self.device
        )
        ...
        for step in range(self.opts.max_episode_len):
            ...
            pano_img_feat, navigable_feat, viewpoints_indices = super(
                PanoSeq2SeqAgent, self
            ).pano_navigable_feat(obs, ended)
            viewpoints, navigable_index, target_index = viewpoints_indices
            ...
            # forward pass the network
            h_t, c_t, ctx_attn, logit, navigable_mask = self.model(
                pano_img_feat,
                navigable_feat,
                pre_feat,
                h_t,
    ...
    def model_seen_step(
        ...
        pano_img_feat, navigable_feat, viewpoints_indices = super(
            PanoSeq2SeqAgent, self
        ).pano_navigable_feat(tmp_obs, ended)
        viewpoints, navigable_index, target_index = viewpoints_indices
        ...
        (
            ...
            tmp_ctx,
            tmp_pre_ctx_attend,
            navigable_index=navigable_index,
    ...
    def sample_progress_inference(self, beam_size=5):
        ...
        for k in range(batch_size):
            ...
            self.visited_state = (
                []
            )  # we probably won't be using this during viewpoint constrained beam search
            ...
            tmp_ctx = ctx[k : k + 1].expand(
                beam_size, ctx.size(1), ctx.size(2)
            )
            tmp_h_t = h_t[k : k + 1].expand(beam_size, h_t.size(1))
            tmp_c_t = c_t[k : k + 1].expand(beam_size, c_t.size(1))
            tmp_ctx_mask = ctx_mask[k : k + 1].expand(
                beam_size, ctx_mask.size(1)
            )
            tmp_pre_feat = torch.zeros(
                beam_size, obs[0]["feature"].shape[1]
            ).to(self.device)
            tmp_question = h_t[k : k + 1].expand(beam_size, h_t.size(1))
            tmp_pre_ctx_attend = torch.zeros(
                beam_size, self.opts.rnn_hidden_size
            ).to(self.device)
            tmp_seen_feat = torch.zeros(
                beam_size, 1, self.opts.rnn_hidden_size
            ).to(self.device)
            ...
            pano_img_feat, navigable_feat, viewpoints_indices = super(
                PanoSeq2SeqAgent, self
            ).pano_navigable_feat(tmp_obs, tmp_ended)
            viewpoints, navigable_index, target_index = viewpoints_indices
            pano_img_feat = pano_img_feat.to(self.device)
            navigable_feat = navigable_feat.to(self.device)
            ...
            self.done_beams[k] = self.progress_inference(
                k,
                logprobs,
                tmp_obs,
                traj,
            ...
            assert (
                self.done_beams[k]["traj"]["path"][-1]
                == self.all_visited_traj[-1]
            ), "Recorded all visited viewpoint does not end up with the same viewpoint as before"
            self.done_beams[k]["traj"]["path"] = self.all_visited_traj

            batch_traj.append(self.done_beams[k]["traj"])
        return batch_traj

    def progress_inference(
        self,
        batch_idx,
        logprobs,
        tmp_obs,
        ...
        beam_seq = torch.zeros(self.opts.max_episode_len, beam_size).to(
            self.device
        )
        beam_seq_logprobs = torch.zeros(
            self.opts.max_episode_len, beam_size
        ).to(self.device)
        beam_logprobs_sum = torch.zeros(beam_size).to(
            self.device
        ...
        for step in range(self.opts.max_episode_len):
            ...
            pano_img_feat, navigable_feat, viewpoints_indices = super(
                PanoSeq2SeqAgent, self
            ).pano_navigable_feat(new_obs, new_ended)
            viewpoints, navigable_index, target_index = viewpoints_indices
            pano_img_feat = pano_img_feat.to(self.device)
            navigable_feat = navigable_feat.to(self.device)
            ...
            if not need_rollback:
                ...
                if self.all_visited_traj == []:
                    ...
                    assert (
                        self.all_visited_traj[-1] == current_path[-2]
                    ), "the previous viewpoint does not match with recorded"
                ...
                for record_i in range(row_max):
                    ...
                    if not new_ended[record_i]:
                        if (
                            current_path[-2:] != self.all_visited_traj[-2:]
                            and current_path[-2:][::-1]
                            != self.all_visited_traj[-2:]
                        ):
                            self.all_visited_traj.append(current_path[-1])
                            self.all_visited_traj.append(current_path[-2])

                    elif (
                        self.all_visited_traj[-2] != self.all_visited_traj[-1]
                    ):  # if it's ended, record the last ended action, also avoid duplicated trajectories
                        self.all_visited_traj.append(current_path[-1])

                    if len(self.all_visited_traj) > 2:
                        if (
                            self.all_visited_traj[-3]
                            == self.all_visited_traj[-2]
                        ):
                            print("what is up?")
            ...
            last_recorded = np.repeat(
                new_last_recorded[row_max : row_max + 1], beam_size
            )
            viewpoints = viewpoints[row_max : row_max + 1] * beam_size
            navigable_index = (
                navigable_index[row_max : row_max + 1] * beam_size
            )
            navigable_feat = navigable_feat[row_max : row_max + 1].repeat(
                beam_size, 1, 1
            )
            tmp_pre_feat = new_pre_feat[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            ...
            tmp_img_attn = new_img_attn[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            tmp_ctx_attn = new_ctx_attn[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            tmp_ctx_mask = new_ctx_mask[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            tmp_seen_feat = new_seen_feat[row_max : row_max + 1].repeat(
                beam_size, 1, 1
            )
            tmp_question = new_question[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            tmp_pre_ctx_attend = new_pre_ctx_attend[
                row_max : row_max + 1
            ].repeat(beam_size, 1)
            logprobs = new_logprobs[row_max : row_max + 1].repeat(beam_size, 1)
            log_values = new_log_values[row_max : row_max + 1].repeat(
                beam_size, 1
            )
            # =====================================================================
            ...
            if last_recorded.all() or step == self.opts.max_episode_len - 1:
                final_beam = {"traj": traj[0]}
                break

        return final_beam

    def progress_inference_step(
        self,
        visited_state,
        batch_idx,
        scan_id,
        ...
        while beam_idx < beam_size and cand_idx < len(candidates):
            ...
            if action >= 1 and v["p"] != -float("inf"):
                ...
                beam_seq[step, beam_idx] = v[
                    "c"
                ]  # c'th word is the continuation, i.e. the navigable direction selected
            ...
            next_headings.append(
                tmp_obs[v["q"]]["navigableLocations"][
                    next_viewpoints[beam_idx]
                ]["heading"]
            )
            ...
            if step >= 1:
                ...
                beam_seq_logprobs[:step, beam_idx] = beam_seq_logprobs_prev[
                    :, v["q"]
                ]

            # append new end terminal at the end of this beam
            beam_seq_logprobs[step, beam_idx] = v["r"]  # the raw logprob here
            beam_logprobs_sum[beam_idx] = v[
                "p"
        ...
        if len(next_viewpoints) < beam_size:
            ...
            beam_seq[:, -missing_length:] = (
                beam_seq[:, replicate_idx]
                .unsqueeze(1)
                .expand(-1, missing_length)
            )
            beam_seq_logprobs[:, -missing_length:] = (
                beam_seq_logprobs[:, replicate_idx]
                .unsqueeze(1)
                .expand(-1, missing_length)
            )
            beam_logprobs_sum[-missing_length:] = beam_logprobs_sum[
                replicate_idx
            ].expand(missing_length)

            new_ended[-missing_length:] = [
                new_ended[replicate_idx]
            ] * missing_length
            new_last_recorded[-missing_length:] = [
                new_last_recorded[replicate_idx]
            ] * missing_length

            new_pre_feat[-missing_length:] = (
                new_pre_feat[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )
            new_tmp_h_t[-missing_length:] = (
                new_tmp_h_t[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )
            new_tmp_c_t[-missing_length:] = (
                new_tmp_c_t[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )
            new_tmp_ctx[-missing_length:] = (
                new_tmp_ctx[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1, -1)
            )
            new_tmp_img_attn[-missing_length:] = (
                new_tmp_img_attn[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )
            new_tmp_ctx_attn[-missing_length:] = (
                new_tmp_ctx_attn[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )
            new_tmp_ctx_mask[-missing_length:] = (
                new_tmp_ctx_mask[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )

            new_value[-missing_length:] = (
                new_value[replicate_idx]
                .unsqueeze(0)
                .expand(missing_length, -1)
            )

        # move within the env
        new_tmp_obs = self.env.teleport_beam(
            batch_idx, scan_id, next_viewpoints, next_headings
        )

        for vix, ob in enumerate(new_tmp_obs):
            if not new_ended[vix] or not new_last_recorded[vix]:
                new_traj[vix]["path"].append(
                    (ob["viewpoint"], ob["heading"], ob["elevation"])
                )
                dist = super(PanoSeq2SeqAgent, self)._get_distance(ob)
                new_traj[vix]["distance"].append(dist)
                new_traj[vix]["img_attn"].append(
                    new_tmp_img_attn[vix].detach().cpu().numpy().tolist()
                )
                new_traj[vix]["ctx_attn"].append(
                    new_tmp_ctx_attn[vix].detach().cpu().numpy().tolist()
                )
                new_traj[vix]["value"].append(
                    new_value[vix].detach().cpu().item()
                )
    ...
    def beam_search(
        ...
        def beam_step_state_factored(
            ...
            beam_logprobs_sum,
            state,
            logprobsf_value=None,
        ):
            if logprobsf_value is not None:
                ys, ix = torch.sort(
                    logprobsf_value.repeat(1, logprobsf.size(1)) + logprobsf,
                    1,
                    True,
                )
            else:
            ...
            num_candidate_routes = (
                1 if self.opts.progress_inference else beam_size
            )
            while beam_idx < num_candidate_routes and cand_idx < len(
                candidates
            ):
                v = candidates[cand_idx]
                ...
                if action >= 1 and v["p"] != -float("inf"):
                    ...
                    next_heading = tmp_obs[v["q"]]["navigableLocations"][
                        next_viewpoint
                    ]["heading"]
                    ...
                    if (next_viewpoint, next_heading) not in visited_state:
                        ...
                        next_viewpoint_idx.append(
                            navigable_index[v["q"]][action - 1]
                        )
                        new_pre_feat[beam_idx] = navigable_feat[
                            v["q"], action, :
                        ]
                        next_viewpoints.append(viewpoints[v["q"]][action])
                        beam_seq[step, beam_idx] = v[
                ...
                # append new end terminal at the end of this beam
                beam_seq_logprobs[step, beam_idx] = v[
                    "r"
            ...
            if len(next_viewpoints) < beam_size:
                ...
                for _ in range(missing_length):
                    next_viewpoints.append(next_viewpoints[replicate_idx])
                    new_navigable_index.append(
                        new_navigable_index[replicate_idx]
                    )
                    next_headings.append(next_headings[replicate_idx])
                    next_viewpoint_idx.append(
                        next_viewpoint_idx[replicate_idx]
                    )
                ...
                beam_logprobs_sum[-missing_length:] = torch.zeros(
                    missing_length
                ).fill_(-float("inf"))
            ...
            for vix, ob in enumerate(new_tmp_obs):
                if not new_ended[vix]:
                    ...
                    new_traj[vix]["navigable_idx"].append(
                        new_navigable_index[vix]
                    )
                    new_traj[vix]["viewpoint_idx"].append(
                        next_viewpoint_idx[vix]
                    )
                    new_traj[vix]["length"] += 1
                    new_last_recorded[vix] = True if new_ended[vix] else False
        ...
        for step in range(self.opts.max_episode_len):
            ...
            for vix in range(beam_size):
                ...
                if current_path not in self.all_visited_traj and beam_logprobs_sum[
                    vix
                ] != -float(
                    "inf"
                ):
                    if self.all_visited_traj != []:
                        ...
                        for vs_idx, visited_states in enumerate(
                            self.all_visited_traj
                        ):

                            insert_idx = -1
                            for vp_idx, (viewpoint, _, _) in enumerate(
                                visited_states
                            ):
                        ...
                        vs_insert_idx, max_insert_idx, = max(
                            insert_idx_set, key=lambda item: item[1]
                        )
                        self.all_visited_traj.insert(
                            vs_insert_idx, current_path
                        )
                ...
                if beam_logprobs_sum[vix] != -float("inf"):
                    if (
                        beam_seq[step, vix] == 0
                        or step == self.opts.max_episode_len - 1
                    ):
                        final_beam = {
