...
class PanoSeq2SeqTrainer:
    ...
    def train(self, epoch, train_env, tb_logger=None):
        ...
        for iter in range(1, self.train_iters_epoch + 1):
            ...
            if self.agent.value_loss is not None:
                val_losses.update(
                    self.agent.value_loss.item(), self.opts.batch_size
                )

            if self.agent.val_acc is not None:
                val_acces.update(
                    np.mean(self.agent.val_acc), self.opts.batch_size
                )
            ...
            if tb_logger and iter % 10 == 0:
                ...
                tb_logger.add_scalar(
                    "train/dist_from_goal", dist_from_goal, current_iter
                )
                tb_logger.add_scalar("train/movements", movement, current_iter)
                if self.agent.value_loss is not None:
                    tb_logger.add_scalar(
                        "train/value_loss", self.agent.value_loss, current_iter
                    )
        ...
        if tb_logger:
            ...
            tb_logger.add_scalar(
                "epoch/train/dist_from_goal", dists.avg, epoch
            )
            tb_logger.add_scalar("epoch/train/movements", movements.avg, epoch)
            if self.agent.value_loss is not None:
                tb_logger.add_scalar(
                    "epoch/train/val_loss", val_losses.avg, epoch
                )
            if self.agent.val_acc is not None:
                tb_logger.add_scalar(
                    "epoch/train/val_acc", val_acces.avg, epoch
                )
    ...
    def eval(self, epoch, val_env, tb_logger=None):
            ...
            while True:
                ...
                if self.opts.progress_inference:
                    ...
                    if tb_logger and iter % 10 == 0:
                        ...
                        tb_logger.add_scalar(
                            "{}/loss".format(env_name), loss, current_iter
                        )
                        tb_logger.add_scalar(
        ...
        if tb_logger:
            tb_logger.add_scalar(
                "epoch/{}/loss".format(env_name), losses.avg, epoch
            )
            tb_logger.add_scalar(
                "epoch/{}/dist_from_goal".format(env_name), dists.avg, epoch
            )
            tb_logger.add_scalar(
                "epoch/{}/movements".format(env_name), movements.avg, epoch
            )
            if self.agent.value_loss is not None:
                tb_logger.add_scalar(
                    "epoch/{}/val_loss".format(env_name), val_losses.avg, epoch
                )
            if self.agent.val_acc is not None:
                tb_logger.add_scalar(
                    "epoch/{}/val_acc".format(env_name), val_acces.avg, epoch
                )
        ...
        for metric, val in score_summary.items():
            ...
            if tb_logger:
                tb_logger.add_scalar(
                    "score/{}/{}".format(env_name, metric), val, epoch
                )
        print(result_str)
