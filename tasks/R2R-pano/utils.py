...
def load_datasets(splits, opts=None):
    ...
    for split in splits:
        ...
        if split == "synthetic":
            with open(
                "tasks/R2R-pano/data/R2R_literal_speaker_data_augmentation_paths.json"
            ) as f:
                data += json.load(f)
        else:
...
class Tokenizer(object):
    ...
    def encode_sentence(self, sentence):
        ...
        if self.remove_punctuation:
            splited = [
                word for word in splited if word not in string.punctuation
            ]
...
def save_checkpoint(
    state, is_best, checkpoint_dir="checkpoints/", name="checkpoint"
):
    ...
    if os.path.isfile(opts.resume):
        if is_experiment():
            ...
            checkpoint = torch.load(
                opts.resume, map_location=lambda storage, loc: storage
            )
