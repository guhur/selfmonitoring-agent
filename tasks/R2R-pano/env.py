...
class R2RPanoBatch:
    ...
    def _shortest_path_action(self, state, goalViewpointId):
        ...
        for i, loc in enumerate(state.navigableLocations):
            if loc.viewpointId == nextViewpointId:
                ...
                if loc.rel_heading > math.pi / 6.0:
                    ...
                    and state.viewIndex // 12 < 2
                ):
                    return (0, 0, 1)  # Look up
                elif (
                    loc.rel_elevation < -math.pi / 6.0
                    and state.viewIndex // 12 > 0
                ):
        ...
        target_rel = (
            self.graphs[state.scanId].node[nextViewpointId]["position"]
        ...
        if (
            ...
            and state.heading - target_heading < math.pi
        ):
            return (0, -1, 0)  # Turn left
        if (
            target_heading > state.heading
            and target_heading - state.heading > math.pi
        ):
            return (0, -1, 0)  # Turn left
        return (0, 1, 0)  # Turn right
    ...
    def _pano_navigable(self, state, goalViewpointId):
        ...
        navigable_graph = self.graphs[state.scanId].adj[
            state.location.viewpointId
        ]
        teacher_path = self.paths[state.scanId][state.location.viewpointId][
            goalViewpointId
        ]
    ...
    def heading_elevation_feat(self, state, horizon_views=12, tile=32):
        ...
        feat = np.stack(
            [rel_sin_phi, rel_cos_phi, rel_sin_theta, rel_cos_theta], axis=0
        )
    ...
    def shortest_path_to_gt_traj(self, state, gt_path):
        ...
        min_steps = 100
        min_distance = 100
        current_distance = self.distances[state.scanId][
            state.location.viewpointId
        ][gt_path[-1]]
        ...
        if current_distance != 0:
            for gt_viewpoint in gt_path:
                steps = len(
                    self.paths[state.scanId][state.location.viewpointId][
                        gt_viewpoint
                    ]
                )
                next_distance = self.distances[state.scanId][gt_viewpoint][
                    gt_path[-1]
                ]
    ...
    def _get_obs(self):
        ...
        for i, (feature, state) in enumerate(self.env.getStates()):
            ...
            if self.opts.follow_gt_traj:
                goal_viewpoint = self.shortest_path_to_gt_traj(
                    state, item["path"]
                )
            else:
            ...
            navigable, gt_viewpoint_idx = self._pano_navigable(
                state, goal_viewpoint
            )
            ...
            obs.append(
                {
                    ...
                    "teacher": item["path"],
                    "new_teacher": self.paths[state.scanId][
                        state.location.viewpointId
                    ][item["path"][-1]],
    ...
    def step(self, scanIds, viewpointIds, headings):
        def rotate_to_target_heading(target_heading, state):
            if target_heading < 0:
                target_heading += 2.0 * math.pi
            if (
                abs(target_heading - state.heading) * 180 / math.pi < 15
                or abs(target_heading - state.heading) * 180 / math.pi > 345
            ):  # if the target relative heading is less than 15 degree, stop rotating
                return (0, 0, 0)
            if (
                state.heading > target_heading
                and state.heading - target_heading < math.pi
    ...
    def teleport_beam(self, batch_idx, scanIds, viewpointIds, headings):
        ...
        for sim in self.env.sims[:beam_size]:
            state = sim.getState()
            long_id = self.env._make_id(
                state.scanId, state.location.viewpointId
            )
            if self.env.features:
                feature = self.env.features[
                    long_id
                ]  # features.shape = (36, 2048)
                feature_states.append((feature, state))
            else:
                feature_states.append((None, state))
